...
def write_stack_cells(layout, design=None, plan=None, log=None):
    ...
    for entry in plan:
        ...
        #- write it out. An attribute nothing reads is not a netlist,
        #- and LVS needs a file. The fingerprint goes in a comment so a
        #- regenerated stack can be compared without re-deriving it.
        import os as _os
        dirname = getattr(layout, "dirname", "") or ""
        if dirname:
            try:
                with open(_os.path.join(dirname, name + ".spice"), "w") as fo:
                    fo.write(f"* generated by cicpy write_stack_cells\n")
                    fo.write(f"* fingerprint {fp}\n")
                    fo.write(f"* DO NOT EDIT -- regenerated every run from\n")
                    fo.write(f"* the parent netlist and the stack grouping\n")
                    fo.write("\n".join(lines) + "\n")
            except Exception as e:
                log.warning(f"{name}: could not write netlist: {e}")
