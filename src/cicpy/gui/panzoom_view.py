...
##  The MIT License (MIT)
######################################################################
"""The shared pan/zoom/fit view.

LayoutView and SchemView are the same UX -- fit on F, Z/ctrl-Z zoom,
arrow-key pan, ctrl-wheel zoom, shift-wheel horizontal pan, right-drag
rubber-band zoom, cursorMoved for the status bar -- and carried two
copies of it. The one real difference is orientation: layout Y grows
up, xschem Y grows down. `Y_UP` states it; the flip and the arrow-pan
sign follow from it.
"""
...
class PanZoomView(QGraphicsView):
    ...
    #- layout Y grows up, xschem Y grows down. The flip and the
    #- arrow-key pan direction both follow from this one flag.
    Y_UP = False
    ...
    def keyPressEvent(self, event):
        k = event.key()
        up = PAN_STEP if self.Y_UP else -PAN_STEP
        if k == Qt.Key_F:
            self.fit()
            return
        if k == Qt.Key_Z:
